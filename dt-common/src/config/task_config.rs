...
use crate::{error::Error, meta::db_enum::DatabaseType};
...
impl TaskConfig {
    ...
    fn load_extractor_config(ini: &Ini) -> Result<ExtractorConfig, Error> {
        let basic_type = ini.get("basic", "type");
        match basic_type {
            Some(type_value) => match type_value.as_str() {
                "struct" => Ok(ExtractorConfig::BasicStruct {
                    db_type: DatabaseType::from_str(
                        ini.get("extractor", "db_type").unwrap().as_str(),
                    )
                    .unwrap(),
    ...
    fn load_sinker_config(ini: &Ini) -> Result<SinkerConfig, Error> {
        ...
        match basic_type {
            Some(type_value) => match type_value.as_str() {
                "struct" => Ok(SinkerConfig::BasicStruct {
                    db_type: DatabaseType::from_str(ini.get("sinker", "db_type").unwrap().as_str())
                        .unwrap(),
    ...
    fn load_filter_config(ini: &Ini) -> Result<FilterConfig, Error> {
        ...
        match basic_type {
            ...
            None => {
                let extractor_type = ini.get("extractor", "type").unwrap();
                match extractor_type.as_str() {
                    "mysql_snapshot" | "mysql_cdc" | "pg_snapshot" | "pg_cdc" => {
                        Ok(FilterConfig::Rdb {
                            do_dbs: ini.get("filter", "do_dbs").unwrap(),
                            ignore_dbs: ini.get("filter", "ignore_dbs").unwrap(),
                            do_tbs: ini.get("filter", "do_tbs").unwrap(),
