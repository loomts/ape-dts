...
pub mod tests_utils {
    ...
    pub const STRUCT_IT_PATH: &str = "/dt-struct/src/test/scripts";
    ...
    pub async fn struct_mock_handler(
        factory: &DatabaseMockUtils,
        script_relative_paths: Vec<&str>,
    ) {
        for script in script_relative_paths {
            _ = factory
                .load_data(format!("{}{}", STRUCT_IT_PATH.to_string(), script).as_str())
                .await;
        }
    }
