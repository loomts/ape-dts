...
impl MongoSinker {
        ...
        for row_data in data.iter_mut() {
            ...
            match row_data.row_type {
                RowType::Insert => {
                    ...
                    if let Some(ColValue::MongoDoc(doc)) = after.remove(Mongo::DOC) {
                ...
                RowType::Delete => {
                    ...
                    if let Some(ColValue::MongoDoc(doc)) = before.remove(Mongo::DOC) {
                ...
                RowType::Update => {
                    ...
                    let query_doc = if let Some(ColValue::MongoDoc(doc)) = before.remove(Mongo::DOC)
                    {
                        Some(doc)
                    } else {
                        None
                    ...
                    let update_doc = if let Some(ColValue::MongoDoc(doc)) = after.remove(Mongo::DOC)
                    {
                        Some(doc)
                    } else {
                        None
    ...
    async fn batch_delete(
        ...
        for i in start_index..start_index + batch_size {
            ...
            if let Some(ColValue::MongoDoc(doc)) = before.get(Mongo::DOC) {
                ids.push(doc.get_object_id(Mongo::ID).unwrap());
            }
        }
    ...
    async fn upsert(
        ...
        let query = doc! {Mongo::ID : query_doc.get_object_id(Mongo::ID).unwrap()};
        let update = doc! {Mongo::SET: update_doc};
