...
-- not match: column
CREATE TABLE struct_check_test_1.not_match_column (id SERIAL PRIMARY KEY,varchar_col2 VARCHAR(255) NOT NULL,char_col CHAR(11));
...
-- not match: check
CREATE TABLE struct_check_test_1.not_match_check (id SERIAL PRIMARY KEY,varchar_col VARCHAR(255) NOT NULL,unique_col VARCHAR(255) UNIQUE,not_null_col VARCHAR(255) NOT NULL,check_col VARCHAR(255) CHECK (char_length(check_col) > 4));
...
CREATE INDEX index_not_match_wrong_column ON struct_check_test_1.full_index_type (unique_col);

CREATE INDEX index_not_match_order ON struct_check_test_1.full_index_type (composite_index_col2, composite_index_col1, composite_index_col3);
