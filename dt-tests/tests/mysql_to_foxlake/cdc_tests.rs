...
mod test {
    ...
    async fn cdc_basic_test() {
        ...
        delete_dir(&s3_client, &bucket, &root_dir).await;
        runner.run_foxlake_test(3000, 10000).await.unwrap();
        check_result(&s3_client, &bucket, &root_dir).await;
    }
    ...
    async fn init_s3_client(task_config_file: &str) -> (Option<S3Client>, String, String) {
